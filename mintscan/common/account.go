...
func GetDelegatorDelegationsLegacy(a *app.App) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		/*
		...
		*/
		...
		resps, err := a.Client.GRPC.GetDelegatorDelegations(r.Context(), accAddr, pageLimit)
		if err != nil {
		...
		res2, err2 := a.Client.GRPC.GetDelegationTotalRewards(r.Context(), accAddr)
		if err2 != nil {
		...
		for _, resp := range resps.DelegationResponses {
			...
			drr, err := a.Client.GRPC.GetDelegationRewards(r.Context(), resp.Delegation.DelegatorAddress, resp.Delegation.ValidatorAddress)
			if err != nil {
				zap.L().Error("failed to get delegator rewards", zap.Error(err))
				errors.ErrServerUnavailable(rw, http.StatusServiceUnavailable)
				return
			...
			// 위임한 검증인의 모니커 조회
			vr, err := a.Client.GRPC.GetValidator(r.Context(), resp.Delegation.ValidatorAddress)
			if err != nil {
